...
def face_3dmm_to_landmarks_np(face_3dmm_params, img_dims, roi_box):
    ...
    pts3d = (
        R
        @ (
            bfm_s.get_bfm().u_base
            + bfm_s.get_bfm().w_shp_base @ alpha_shp
            + bfm_s.get_bfm().w_exp_base @ alpha_exp
        ).reshape(3, -1, order="F")
        + offset
    )
...
def face_landmarks_3d_postprocessing(
    endnodes, device_pre_post_layers=None, *, img_dims=None, gt_images=None, **kwargs
):
    assert img_dims[0] == img_dims[1], "Assumes square input"
    batch_size = tf.shape(endnodes)[0]
    endnodes = tf.reshape(endnodes, [batch_size, -1])
    face_3dmm_params = (
        endnodes * TDDFA_RESCALE_PARAMS["std"] + TDDFA_RESCALE_PARAMS["mean"]
    )
    roi_box = gt_images.get(
        "roi_box", tf.tile([[0, 0, img_dims[1], img_dims[0]]], (batch_size, 1))
    )
    ptds3d = tf.numpy_function(
        face_3dmm_to_landmarks_batch, [face_3dmm_params, img_dims, roi_box], tf.float32
    )
...
def visualize_face_landmarks_3d_result(logits, image, **kwargs):
    ...
    if box is not None:
        img = cv2.rectangle(
            img, (box[0], box[1]), (box[2], box[3]), color=(0, 255, 0), thickness=1
        )
    return img
