...
class FaceDetectionPostProc(object):
    ...
    def collect_box_class_predictions(self, output_branches):
        ...
        assert (
            len(sorted_output_branches) % num_branches == 0
        ), "All branches must have the same number of output nodes"
        num_output_nodes_per_branch = len(sorted_output_branches) // num_branches
        for branch_index in range(
            0, len(sorted_output_branches), num_output_nodes_per_branch
        ):
            num_of_batches, _, _, _ = tf.unstack(
                tf.shape(sorted_output_branches[branch_index])
            )
            box_predictors_list.append(
                tf.reshape(
                    sorted_output_branches[branch_index], shape=[num_of_batches, -1, 4]
                )
            )
            class_predictors_list.append(
                tf.reshape(
                    sorted_output_branches[branch_index + 1],
            ...
            if num_output_nodes_per_branch > 2:
                ...
                landmarks_predictors_list.append(
                    tf.reshape(
                        sorted_output_branches[branch_index + 2],
        ...
        landmarks_predictors = (
            tf.concat(landmarks_predictors_list, axis=1)
    ...
    def extract_anchors(self, min_sizes, steps):
        feature_maps = [
            [
                int(np.ceil(self._image_dims[0] / step)),
                int(np.ceil(self._image_dims[1] / step)),
            ]
            for step in steps
        ]
    ...
    def _decode_landmarks(self, landmarks_detections, anchors):
        return tf.concat(
            (
                anchors[:, :2]
                + landmarks_detections[:, :2] / self.SCALE_FACTORS[0] * anchors[:, 2:],
                anchors[:, :2]
                + landmarks_detections[:, 2:4] / self.SCALE_FACTORS[0] * anchors[:, 2:],
                anchors[:, :2]
                + landmarks_detections[:, 4:6] / self.SCALE_FACTORS[0] * anchors[:, 2:],
                anchors[:, :2]
                + landmarks_detections[:, 6:8] / self.SCALE_FACTORS[0] * anchors[:, 2:],
                anchors[:, :2]
                + landmarks_detections[:, 8:10]
    ...
    def _decode_boxes(self, box_detections, anchors):
        boxes = tf.concat(
            (
                ...
                anchors[:, 2:] * tf.exp(box_detections[:, 2:] / self.SCALE_FACTORS[1]),
            ),
            1,
        )
    ...
    def tf_postproc(self, endnodes):
        with tf.name_scope("Postprocessor"):
            box_predictions, classes_predictions, landmarks_predictors = (
                self.collect_box_class_predictions(endnodes)
            )
            additional_fields = {}
            ...
            detection_scores = tf.slice(
                classes_predictions_softmax, [0, 0, 1], [-1, -1, -1]
            )

            batch_size, num_proposals = tf.unstack(
                tf.slice(tf.shape(box_predictions), [0], [2])
            )

            tiled_anchor_boxes = tf.tile(
                tf.expand_dims(self._anchors, 0), [batch_size, 1, 1]
            )
            tiled_anchors_boxlist = tf.reshape(tiled_anchor_boxes, [-1, 4])

            decoded_boxes = self._decode_boxes(
                tf.reshape(box_predictions, (-1, 4)), tiled_anchors_boxlist
            )
            ...
            if tf.is_tensor(landmarks_predictors):
                ...
                decoded_landmarks = tf.reshape(
                    decoded_landmarks, [batch_size, num_proposals, 10]
                )
                additional_fields["landmarks"] = decoded_landmarks

            detection_boxes = tf.identity(
                tf.expand_dims(detection_boxes, axis=[2]), "raw_box_locations"
            )
            (
                nmsed_boxes,
                nmsed_scores,
